...
//User must have bUnwarpJ and the DisplacementMaps macro  and the 
//StrainMapsFromDisplacementMaps macro saved in the macro folder.
...
for (j = 1; j <= (imageSlices-frameIncrement); j=j+interval){
	//if (c <= nSets){
		...
		//call("bunwarpj.bUnwarpJ_.convertToRawTransformationMacro", tempDir+sourceImage, tempDir+targetImage,inverseTransSave, inverseTransSave);
	
		selectWindow(sourceImage);
		close();
		selectWindow(targetImage);
		close();
		savePath = runMacro("DisplacementMaps", directTransSave);
		runMacro("StrainMapsFromDisplacementMaps", savePath);
		selectWindow("rawX"); close();
		selectWindow("rawY"); close();
		run("Collect Garbage");
		//savePath = runMacro("DisplacementMaps", inverseTransSave);
		//runMacro("StrainMapsFromDisplacementMaps", savePath);
		//selectWindow("Result of rawX"); close();
		//selectWindow("Result of rawY"); close();
		//c++;
	//}
}
...
for (m= 1; m <= (imageSlices-frameIncrement); m=m+interval){
	//if(c2 <=nSets){
		...
		epsyxName = directTransFilePrefix+"_epsyx.tif";
		epsyxPath = directTransFile+"_epsyx.tif";
		open(epsyxPath);
		imageCalculator("Multiply create 32-bit", "mask.tif",epsyxName);
		selectWindow("Result of mask.tif");
		run("Divide...", "value=255");
		save(saveDir+epsyxName); close();
		selectWindow(epsyxName); close();

		eps1Name = directTransFilePrefix+"_eps1.tif";
		eps1Path = directTransFile+"_eps1.tif";
		open(eps1Path);
		imageCalculator("Multiply create 32-bit", "mask.tif",eps1Name);
		selectWindow("Result of mask.tif");
		run("Divide...", "value=255");
		save(saveDir+eps1Name); close();
		selectWindow(eps1Name); close();

		eps2Name = directTransFilePrefix+"_eps2.tif";
		eps2Path = directTransFile+"_eps2.tif";
		open(eps2Path);
		imageCalculator("Multiply create 32-bit", "mask.tif",eps2Name);
		selectWindow("Result of mask.tif");
		run("Divide...", "value=255");
		save(saveDir+eps2Name); close();
		selectWindow(eps2Name); close();

		thetaName = directTransFilePrefix+"_theta.tif";
		thetaPath = directTransFile+"_theta.tif";
		open(thetaPath);
		imageCalculator("Multiply create 32-bit", "mask.tif", thetaName);
		selectWindow("Result of mask.tif");
		run("Divide...", "value=255");
		save(saveDir+thetaName); close();
		selectWindow(thetaName); close();
...
run("Image Sequence...", "open=["+saveDir+thetaName+"] number=&image_count starting=1 increment=1 scale=100 file=t_transf_deltaX.tif or=[] sort");
rename("deltax");
save(dirName+"deltax.tif"); close();


run("Image Sequence...", "open=["+saveDir+thetaName+"] number=&image_count starting=1 increment=1 scale=100 file=t_transf_deltaY.tif or=[] sort");
rename("deltay");
save(dirName+"deltay.tif"); close();


run("Image Sequence...", "open=["+saveDir+thetaName+"] number=&image_count starting=1 increment=1 scale=100 file=t_transf_epsx.tif or=[] sort");
rename("epsx");
...
run("Image Sequence...", "open=["+saveDir+thetaName+"] number=&image_count starting=1 increment=1 scale=100 file=t_transf_epsy.tif or=[] sort");
rename("epsy");
getStatistics(area, mean, min, max, std, histogram);
if(abs(max)>abs(min)){
	upper = max;
...
run("Image Sequence...", "open=["+saveDir+thetaName+"] number=&image_count starting=1 increment=1 scale=100 file=t_transf_epsxy.tif or=[] sort");
rename("epsxy");
getStatistics(area, mean, min, max, std, histogram);
if(abs(max)>abs(min)){
	upper = max;
...
run("Image Sequence...", "open=["+saveDir+thetaName+"] number=&image_count starting=1 increment=1 scale=100 file=t_transf_epsyx.tif or=[] sort");
rename("epsyx");
getStatistics(area, mean, min, max, std, histogram);
if(abs(max)>abs(min)){
	upper = max;
...
if( getPixel(0,0)==0 && getPixel(0,1)==0){
	...
	IJ.log("Unable to balance histogram for epsyx");
}


run("Image Sequence...", "open=["+saveDir+thetaName+"] number=&image_count starting=1 increment=1 scale=100 file=t_transf_eps1.tif or=[] sort");
rename("eps1");
getStatistics(area, mean, min, max, std, histogram);
if(abs(max)>abs(min)){
	upper = max;
...
if( getPixel(0,0)==0 && getPixel(0,1)==0){
	...
	IJ.log("Unable to balance histogram for eps1");
}
//setAutoThreshold("Default");
//save(dirName+"eps1_test.tif"); //close();
//image_count++;

run("Image Sequence...", "open=["+saveDir+thetaName+"] number=&image_count starting=1 increment=1 scale=100 file=t_transf_eps2.tif or=[] sort");
rename("eps2");
getStatistics(area, mean, min, max, std, histogram);
if(abs(max)>abs(min)){
	upper = max;
...
if( getPixel(0,0)==0 && getPixel(0,1)==0){
	...
	IJ.log("Unable to balance histogram for eps2");
}
//setAutoThreshold("Default");
//save(dirName+"eps2_test.tif"); //close();
//image_count++;

run("Image Sequence...", "open=["+saveDir+thetaName+"] number=&image_count starting=1 increment=1 scale=100 file=t_transf_theta.tif or=[] sort");
rename("theta");
getStatistics(area, mean, min, max, std, histogram);
if(abs(max)>abs(min)){
	upper = max;
...
if( getPixel(0,0)==0 && getPixel(0,1)==0){
	...
	IJ.log("Unable to balance histogram for theta");
}
setAutoThreshold("Default");
save(dirName+"theta.tif"); close();
//image_count++;

run("Concatenate...", "  title=[eps_xy] image1=epsx image2=epsy image3=epsxy image4=epsyx image5=[-- None --]");
setAutoThreshold("Default");
...
selectWindow("eps_xy");
save(dirName+"epsyx.tif"); close();

run("Concatenate...", "  title=[eps_prcpl] image1=eps1 image2=eps2 image3=[-- None --]");
setAutoThreshold("Default");
run("Make Substack...", "delete slices=1-"+nSets);
save(dirName+"eps1.tif"); close();
selectWindow("eps_prcpl");
save(dirName+"eps2.tif"); close();
